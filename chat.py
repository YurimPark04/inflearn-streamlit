...
if user_question := st.chat_input(placeholder="궁금한 내용을 말해주세요"):  # 채팅을 입력할때마다 실행
    ...
    with st.spinner("답변을 생성하는 중입니다"):
        ...
        with st.chat_message("ai"):  
            ...
            # write 는 파라미터로 string을 받게 된다. -> write_stream으로 바꿔줘야 한다. 
            # write_stream 으로 변경해야 Generator를 받게 됨 - string 을 리턴 : 최종으로 나온 전체 답변을 넣어줘야 다음 채팅이 들어올 때 에러가 안난다 (채팅할때마다 ui자동으로 그려지는 문제)
            ai_message = st.write_stream(ai_response)  # 채팅을 입력할때마다, 전체 코드가 처음부터 실행되는 문제가 생김 (line by line으로 확인)

        # 최종적으로 사용할 ai_message를 받아준다 
